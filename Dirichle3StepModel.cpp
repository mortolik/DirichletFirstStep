...
void Dirichle3StepModel::setup(int n, int m, double omega, double eps, int maxIter)
{
    m_n = n;
    m_m = m;
    m_omega = omega;
...
double Dirichle3StepModel::f(double x, double y) const
{
    return -qExp(-x * y * y);
}

double Dirichle3StepModel::fTest(double x, double y) const
{
    return M_PI*M_PI*(x*x + y*y) * qSin(M_PI * x * y);
}

double Dirichle3StepModel::uStar(double x, double y) const
{
    return qSin(M_PI * x * y);
}

double Dirichle3StepModel::mu1(double y) const
{
    return (y - 2.0) * (y - 3.0);
}

double Dirichle3StepModel::mu2(double y) const
{
    return y * (y - 2.0) * (y - 3.0);
}

double Dirichle3StepModel::mu3(double x) const
{
    return (x - 1.0) * (x - 2.0);
}

double Dirichle3StepModel::mu4(double x) const
{
    return x * (x - 1.0) * (x - 2.0);
}

void Dirichle3StepModel::initialize()
{
...
void Dirichle3StepModel::solveTestProblem()
{
...
double Dirichle3StepModel::computeInitialResidual() const
{
    double maxR = 0.0;
    for (int i = 1; i < m_n; ++i)
    {
...
void Dirichle3StepModel::solveMainProblem()
{
    initializeInterior();
...
QVector<QVector<double>> Dirichle3StepModel::error() const
{
...
double Dirichle3StepModel::maxError() const
{
    double maxErr = 0.0;
...
const QVector<QVector<double>> &Dirichle3StepModel::solution() const
{
    return m_u;
}

const QVector<QVector<double>> &Dirichle3StepModel::exactSolution() const
{
    return m_uExact;
}

void Dirichle3StepModel::initializeInterior()
{
    for (int i = 1; i < m_n; ++i)
    {
        double x = m_a + i * m_h;
...
QVector<QVector<double>> Dirichle3StepModel::compareWithFinerGrid(int finerN, int finerM, double &eps2Out) const
{
    Dirichle3StepModel fineModel;
...
Dirichle3StepModel::ReportData Dirichle3StepModel::generateReportData(bool isTestTask) const
{
    ReportData data;
    data.n = m_n;
    data.m = m_m;
...
QString Dirichle3StepModel::reportString(bool isTestTask) const
{
...
QPair<double, double> Dirichle3StepModel::maxErrorPointCompare() const
{
    Dirichle3StepModel fineModel;
...
QPair<double, double> Dirichle3StepModel::maxErrorPoint() const
{
    double maxErr = 0.0;
    int maxI = 0, maxJ = 0;
...
double Dirichle3StepModel::computeOptimalOmega()
{
...
Dirichle3StepModel::FinerGridResult Dirichle3StepModel::computeFinerGridComparison() const
{
    Dirichle3StepModel fineModel;
    fineModel.setup(m_n * 2, m_m * 2, m_omega, m_eps, m_maxIter);
    fineModel.solveMainProblem();
