...
void DirichletSolverModel2::setup(int n, int m, double omega, double eps, int maxIter)
{
    m_n = n;
    m_m = m;
    m_omega = omega;
...
double DirichletSolverModel2::f(double x, double y) const
{
    return -qExp(-x * y * y);
}

double DirichletSolverModel2::fTest(double x, double y) const
{
    return M_PI*M_PI*(x*x + y*y) * qSin(M_PI * x * y);
}

double DirichletSolverModel2::uStar(double x, double y) const
{
    return qSin(M_PI * x * y);
}

double DirichletSolverModel2::mu1(double y) const
{
    return (y - 2.0) * (y - 3.0);
}

double DirichletSolverModel2::mu2(double y) const
{
    return y * (y - 2.0) * (y - 3.0);
}

double DirichletSolverModel2::mu3(double x) const
{
    return (x - 1.0) * (x - 2.0);
}

double DirichletSolverModel2::mu4(double x) const
{
    return x * (x - 1.0) * (x - 2.0);
}

void DirichletSolverModel2::initialize()
{
...
void DirichletSolverModel2::solveTestProblem()
{
...
double DirichletSolverModel2::computeInitialResidual() const
{
    double maxR = 0.0;
    for (int i = 1; i < m_n; ++i)
    {
...
void DirichletSolverModel2::solveMainProblem()
{
    initializeInterior();
...
QVector<QVector<double>> DirichletSolverModel2::error() const
{
...
double DirichletSolverModel2::maxError() const
{
    double maxErr = 0.0;
...
const QVector<QVector<double>> &DirichletSolverModel2::solution() const
{
    return m_u;
}

const QVector<QVector<double>> &DirichletSolverModel2::exactSolution() const
{
    return m_uExact;
}

void DirichletSolverModel2::initializeInterior()
{
    for (int i = 1; i < m_n; ++i)
    {
        double x = m_a + i * m_h;
...
QVector<QVector<double>> DirichletSolverModel2::compareWithFinerGrid(int finerN, int finerM, double &eps2Out) const
{
    DirichletSolverModel2 fineModel;
...
DirichletSolverModel2::ReportData DirichletSolverModel2::generateReportData(bool isTestTask) const
{
    ReportData data;
    data.n = m_n;
    data.m = m_m;
...
QString DirichletSolverModel2::reportString(bool isTestTask) const
{
...
QPair<double, double> DirichletSolverModel2::maxErrorPointCompare() const
{
    DirichletSolverModel2 fineModel;
...
QPair<double, double> DirichletSolverModel2::maxErrorPoint() const
{
    double maxErr = 0.0;
    int maxI = 0, maxJ = 0;
...
double DirichletSolverModel2::computeOptimalOmega()
{
...
DirichletSolverModel2::FinerGridResult DirichletSolverModel2::computeFinerGridComparison() const
{
    DirichletSolverModel2 fineModel;
    fineModel.setup(m_n * 2, m_m * 2, m_omega, m_eps, m_maxIter);
    fineModel.solveMainProblem();
