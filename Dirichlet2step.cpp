...
Dirichlet2step::Dirichlet2step(QObject *parent)
...
void Dirichlet2step::setup(int n, int m, double omega, double eps, int maxIter)
{
    m_n = n;
    m_m = m;
    m_omega = omega;
...
double Dirichlet2step::f(double x, double y) const
{
    return -qExp(-x * y * y);
}

double Dirichlet2step::fTest(double x, double y) const
{
    return M_PI*M_PI*(x*x + y*y) * qSin(M_PI * x * y);
}

double Dirichlet2step::uStar(double x, double y) const
{
    return qSin(M_PI * x * y);
}

double Dirichlet2step::mu1(double y) const
{
    return (y - 2.0) * (y - 3.0);
}

double Dirichlet2step::mu2(double y) const
{
    return y * (y - 2.0) * (y - 3.0);
}

double Dirichlet2step::mu3(double x) const
{
    return (x - 1.0) * (x - 2.0);
}

double Dirichlet2step::mu4(double x) const
{
    return x * (x - 1.0) * (x - 2.0);
}

void Dirichlet2step::initialize()
{
...
void Dirichlet2step::solveTestProblem()
{
...
double Dirichlet2step::computeInitialResidual() const
{
    double maxR = 0.0;
    for (int i = 1; i < m_n; ++i)
    {
...
void Dirichlet2step::solveMainProblem()
{
    initializeInterior();
...
QVector<QVector<double>> Dirichlet2step::error() const
{
...
double Dirichlet2step::maxError() const
{
    double maxErr = 0.0;
...
const QVector<QVector<double>> &Dirichlet2step::solution() const
{
    return m_u;
}

const QVector<QVector<double>> &Dirichlet2step::exactSolution() const
{
    return m_uExact;
}

void Dirichlet2step::initializeInterior()
{
    for (int i = 1; i < m_n; ++i)
    {
        double x = m_a + i * m_h;
...
QVector<QVector<double>> Dirichlet2step::compareWithFinerGrid(int finerN, int finerM, double &eps2Out) const
{
    Dirichlet2step fineModel;
...
Dirichlet2step::ReportData Dirichlet2step::generateReportData(bool isTestTask) const
{
    ReportData data;
    data.n = m_n;
    data.m = m_m;
...
QString Dirichlet2step::reportString(bool isTestTask) const
{
...
QPair<double, double> Dirichlet2step::maxErrorPointCompare() const
{
    Dirichlet2step fineModel;
...
QPair<double, double> Dirichlet2step::maxErrorPoint() const
{
    double maxErr = 0.0;
    int maxI = 0, maxJ = 0;
...
double Dirichlet2step::computeOptimalOmega()
{
...
Dirichlet2step::FinerGridResult Dirichlet2step::computeFinerGridComparison() const
{
    Dirichlet2step fineModel;
    fineModel.setup(m_n * 2, m_m * 2, m_omega, m_eps, m_maxIter);
    fineModel.solveMainProblem();
